(function () {
  ...
  const state = { demo: normalise(bundled), activeCategory: "All", query: "", remoteLoaded: false, selectedItem: null, cart: JSON.parse(localStorage.getItem(`alpha-demo-cart-${key}`) || "[]") };
  ...
  function interactionFor(d) {
    const cart = ["restaurant", "store", "qr", "electronics"];
    const appointment = ["clinic", "salon", "petcare"];
    const labels = {
      realestate: ["Schedule a site visit", "Site visit request"], coaching: ["Book a demo class", "Demo class registration"],
      local: ["Book this service", "Service booking"], events: ["Request this package", "Event requirement"],
      bikes: ["Check date availability", "Rental availability"], travel: ["Reserve this trip", "Trip reservation"],
      gaming: ["Reserve a gaming slot", "Gaming slot request"], coworking: ["Book a tour or desk", "Workspace booking"]
    };
    if (cart.includes(d.key)) return { mode: "cart", button: "Add to cart", title: "Your cart" };
    if (appointment.includes(d.key)) return { mode: "appointment", button: "Book an appointment", title: "Appointment request" };
    if (labels[d.key]) return { mode: "booking", button: labels[d.key][0], title: labels[d.key][1] };
    return { mode: "enquiry", button: "Enquire about this", title: "Detailed enquiry" };
  }
  ...
  function render() {
    const d = state.demo;
    const interaction = interactionFor(d);
    ...
    document.getElementById("app").innerHTML = `
      <div class="demo-shell">
        <div class="top-note"><a class="back-to-demos" href="../../index.html#services">← All demo websites</a><span>Live demo by <a href="${escapeHtml(config.alphaWebsite || alpha.website)}" target="_blank" rel="noopener">Alpha Web Solutions</a></span><button class="share-button top-share" type="button">Share demo ↗</button></div>
        <header class="demo-header">
          ...
          <nav class="demo-nav"><a href="#home">Home</a><a href="#browse">Explore</a>${industryNav}<a href="#location">Location</a><a href="#faq">FAQ</a>${interaction.mode === "cart" ? '<button class="nav-cta demo-cart-open" type="button">Cart <b id="demo-cart-count">0</b></button>' : `<button class="nav-cta demo-action-open" type="button">${escapeHtml(interaction.button)}</button>`}</nav>
        </header>
        <main>
        ...
        <div class="flow-backdrop" id="flow-backdrop" role="dialog" aria-modal="true" aria-label="${escapeHtml(interaction.title)}">
          <section class="flow-panel">
            <div class="flow-head"><div><span class="demo-eyebrow">${escapeHtml(d.name)}</span><h2 id="flow-title">${escapeHtml(interaction.title)}</h2></div><button class="flow-close" type="button" aria-label="Close">×</button></div>
  ...
  function openModal(id) {
    ...
    const interaction = interactionFor(state.demo);
    ...
    const sourceMarkup = item.source_url
      ...
      : "";
    document.getElementById("modal-content").innerHTML = `<button class="modal-image" type="button" data-image-src="${escapeHtml(item.image_url)}" data-image-title="${escapeHtml(item.title)}" aria-label="Enlarge ${escapeHtml(item.title)} image"><img src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.title)}"><span class="image-hint">Tap to enlarge</span></button><div class="modal-content"><button class="modal-close" aria-label="Close details">×</button><div class="item-category">${escapeHtml(item.category)} • ${escapeHtml(item.badge || "Featured")}</div><h2>${escapeHtml(item.title)}</h2>${detailMarkup}<div class="detail-facts"><span><small>Price</small><b>${escapeHtml(item.price || "Enquire")}</b></span><span><small>Category</small><b>${escapeHtml(item.category)}</b></span><span><small>Status</small><b>${escapeHtml(item.badge || "Available")}</b></span></div>${sourceMarkup}<div class="modal-actions"><button class="demo-button primary item-primary-action" type="button">${escapeHtml(interaction.button)} <span>→</span></button><button class="demo-button secondary modal-close-secondary" type="button">Continue browsing</button></div></div>`;
    ...
    backdrop.querySelector(".item-primary-action").addEventListener("click", () => {
      if (interaction.mode === "cart") addToCart(item.id);
      else { closeModal(); openCustomerFlow(item.id); }
    });
  ...
  function addToCart(id) {
    ...
    localStorage.setItem(`alpha-demo-cart-${key}`, JSON.stringify(state.cart));
    closeModal();
    renderCart();
    openCustomerFlow();
  ...
  function cartTotal() {
    return state.cart.reduce((sum, entry) => {
      const item = state.demo.items.find((candidate) => candidate.id === entry.id);
      return sum + numericPrice(item?.price) * entry.qty;
    }, 0);
  }

  function renderCart() {
    const count = state.cart.reduce((sum, entry) => sum + entry.qty, 0);
    const badge = document.getElementById("demo-cart-count");
    if (badge) badge.textContent = count;
    if (interactionFor(state.demo).mode !== "cart") return;
    const target = document.getElementById("flow-content");
    if (!target) return;
    target.innerHTML = count ? `
      <div class="flow-cart-lines">${state.cart.map((entry) => {
        const item = state.demo.items.find((candidate) => candidate.id === entry.id);
        return `<article class="flow-cart-line"><img src="${escapeHtml(item?.image_url || "")}" alt=""><div><strong>${escapeHtml(item?.title || "Item")}</strong><small>${escapeHtml(item?.price || "Enquire")} each</small><div class="flow-qty"><button type="button" data-cart-qty="-1" data-cart-id="${escapeHtml(entry.id)}">−</button><b>${entry.qty}</b><button type="button" data-cart-qty="1" data-cart-id="${escapeHtml(entry.id)}">+</button></div></div><button class="flow-remove" type="button" data-cart-remove="${escapeHtml(entry.id)}">Remove</button></article>`;
      }).join("")}</div>
      <div class="flow-total"><span>Estimated total</span><strong>${money(cartTotal())}</strong></div>
      <form class="customer-flow-form" id="cart-order-form">
        <div class="flow-form-grid"><label>Name<input name="name" required autocomplete="name"></label><label>Phone / WhatsApp<input name="phone" required inputmode="tel" autocomplete="tel"></label></div>
        <fieldset><legend>Order method</legend><label><input type="radio" name="method" value="Home delivery" checked> Home delivery</label><label><input type="radio" name="method" value="Pickup / takeaway"> Pickup / takeaway</label></fieldset>
        <label>Delivery address / pickup preference<textarea name="address" required placeholder="Full address with PIN code, or preferred pickup time"></textarea></label>
        <label>Notes<textarea name="notes" placeholder="Quantity notes, size, colour, spice level, allergies or other requirements"></textarea></label>
        <button class="demo-button primary full-button" type="submit">Send complete order on WhatsApp <span>↗</span></button>
        <p class="flow-note">Availability, final price and delivery charges are confirmed by the business.</p>
      </form>` : `<div class="flow-empty"><span>◉</span><h3>Your cart is empty</h3><p>Open any item, review its details and add it here.</p><button class="demo-button secondary flow-close-inline" type="button">Continue browsing</button></div>`;
    target.querySelectorAll("[data-cart-qty]").forEach((button) => button.addEventListener("click", () => {
      const entry = state.cart.find((candidate) => candidate.id === button.dataset.cartId);
      if (!entry) return;
      entry.qty += Number(button.dataset.cartQty);
      if (entry.qty <= 0) state.cart = state.cart.filter((candidate) => candidate.id !== entry.id);
      localStorage.setItem(`alpha-demo-cart-${key}`, JSON.stringify(state.cart));
      renderCart();
    }));
    target.querySelectorAll("[data-cart-remove]").forEach((button) => button.addEventListener("click", () => {
      state.cart = state.cart.filter((candidate) => candidate.id !== button.dataset.cartRemove);
      localStorage.setItem(`alpha-demo-cart-${key}`, JSON.stringify(state.cart));
      renderCart();
    }));
    target.querySelector(".flow-close-inline")?.addEventListener("click", closeCustomerFlow);
    target.querySelector("#cart-order-form")?.addEventListener("submit", sendCartOrder);
  }

  function openCustomerFlow(itemId = null) {
    if (itemId) state.selectedItem = itemId;
    const interaction = interactionFor(state.demo);
    document.getElementById("flow-title").textContent = interaction.title;
    if (interaction.mode === "cart") renderCart();
    else renderActionForm();
    document.getElementById("flow-backdrop").classList.add("open");
    document.body.classList.add("no-scroll");
  }

  function closeCustomerFlow() {
    document.getElementById("flow-backdrop").classList.remove("open");
    document.body.classList.remove("no-scroll");
  }

  function renderActionForm() {
    const interaction = interactionFor(state.demo);
    const selected = state.demo.items.find((entry) => entry.id === state.selectedItem) || state.demo.items[0];
    const isAppointment = interaction.mode === "appointment";
    document.getElementById("flow-content").innerHTML = `
      <div class="selected-service"><img src="${escapeHtml(selected?.image_url || state.demo.heroImage)}" alt=""><div><small>Selected ${escapeHtml(selected?.category || "service")}</small><strong>${escapeHtml(selected?.title || state.demo.service)}</strong><span>${escapeHtml(selected?.price || "Price on enquiry")}</span></div></div>
      <form class="customer-flow-form" id="customer-action-form">
        <div class="flow-form-grid"><label>Full name<input name="name" required autocomplete="name"></label><label>Phone / WhatsApp<input name="phone" required inputmode="tel" autocomplete="tel"></label></div>
        ${interaction.mode !== "enquiry" ? `<div class="flow-form-grid"><label>Preferred date<input name="date" type="date" required></label><label>Preferred time<select name="time"><option>Morning</option><option>Afternoon</option><option>Evening</option></select></label></div>` : ""}
        ${isAppointment ? '<label>Reason / service notes<textarea name="requirement" required placeholder="Briefly describe the appointment requirement"></textarea></label>' : '<label>Requirement details<textarea name="requirement" required placeholder="Tell us what you need, number of people, location or any preferences"></textarea></label>'}
        <label>Email (optional)<input name="email" type="email" autocomplete="email"></label>
        <button class="demo-button primary full-button" type="submit">${escapeHtml(interaction.button)} on WhatsApp <span>↗</span></button>
        <p class="flow-note">This sends a request for business confirmation; it is not an automatic confirmed booking.</p>
      </form>`;
    const date = document.querySelector("#customer-action-form [name=date]");
    if (date) date.min = new Date().toISOString().slice(0, 10);
    document.getElementById("customer-action-form").addEventListener("submit", sendActionRequest);
  }

  function sendCartOrder(event) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const lines = state.cart.map((entry, index) => {
      const item = state.demo.items.find((candidate) => candidate.id === entry.id);
      return `${index + 1}. ${item.title}\n   ${item.price} × ${entry.qty} = ${money(numericPrice(item.price) * entry.qty)}`;
    }).join("\n\n");
    const message = `Hello ${state.demo.name},\n\nI would like to place this order:\n\n${lines}\n\nEstimated total: ${money(cartTotal())}\nOrder method: ${data.get("method")}\nName: ${data.get("name")}\nPhone: ${data.get("phone")}\nAddress / pickup: ${data.get("address")}\nNotes: ${data.get("notes") || "None"}\n\nPlease confirm availability, final price and fulfilment time.\n\nDemo powered by Alpha Web Solutions.`;
    window.open(whatsapp(message), "_blank", "noopener");
  }

  function sendActionRequest(event) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const interaction = interactionFor(state.demo);
    const item = state.demo.items.find((entry) => entry.id === state.selectedItem) || state.demo.items[0];
    const schedule = interaction.mode === "enquiry" ? "" : `\nPreferred date: ${data.get("date")}\nPreferred time: ${data.get("time")}`;
    const message = `Hello ${state.demo.name},\n\n${interaction.title}\nSelected: ${item.title}\nPrice: ${item.price}\nName: ${data.get("name")}\nPhone: ${data.get("phone")}\nEmail: ${data.get("email") || "Not provided"}${schedule}\nRequirement: ${data.get("requirement")}\n\nPlease confirm the details.\n\nDemo powered by Alpha Web Solutions.`;
    window.open(whatsapp(message), "_blank", "noopener");
  }

  function wireCustomerFlow() {
    document.querySelectorAll(".demo-cart-open").forEach((button) => button.addEventListener("click", () => openCustomerFlow()));
    document.querySelectorAll(".demo-action-open").forEach((button) => button.addEventListener("click", () => openCustomerFlow()));
    document.querySelector(".flow-close").addEventListener("click", closeCustomerFlow);
    document.getElementById("flow-backdrop").addEventListener("click", (event) => { if (event.target.id === "flow-backdrop") closeCustomerFlow(); });
  }
  ...
  function wireNavigation() {
    ...
    document.querySelectorAll(".share-button").forEach((button) => button.addEventListener("click", sharePage));
